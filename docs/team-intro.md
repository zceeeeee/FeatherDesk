...
| 视觉模块（多模态 LLM） | ✅ |
| 自愈机制（选择器自动提升） | ✅ |
| 插件系统（SkillBase + skills.yaml） | ✅ |
| 事件钩子（EventBus） | ✅ |
| CLI 工具（serve/run/doctor） | ✅ |
| 文档体系（MkDocs + ADR + examples） | ✅ |
| 测试（475 个，全部通过） | ✅ |
| CloakBrowser 反检测集成 | ✅ |
...
| 功能 | 优先级 | 说明 |
|------|--------|------|
| 更多站点适配器 | 高 | 扩充 domains/ 和 interactions/ |
| 脚本持久化 | 中 | AI 生成的脚本可以保存复用 |
...
A: `make test`，475 个测试全部用 mock，不需要真实浏览器。
